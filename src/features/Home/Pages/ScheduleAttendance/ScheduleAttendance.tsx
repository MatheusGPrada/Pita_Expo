...
export const ScheduleAttendance: FC = () => {
  ...
  const mountCardToRender = (filteredData: Array<Attendance>) => {
    ...
    const isTheLastAttendance = (index: number) =>
      filteredData.length === index + 1;
    let previosEnd = "";
    let availableTime = "";
    filteredData.map(
      ({ horario, idAgendamento }: Attendance, index: number) => {
        if (index === 0) {
          let start = "10:00";

          if (
            parseInt(horario.replace(":", ""), 10) -
              parseInt(start.replace(":", ""), 10) >=
            parseInt(selectedServicesTime.replace(":", ""), 10)
          ) {
            while (
              parseInt(start.replace(":", ""), 10) <
              parseInt(horario.substr(0, 5).replace(":", ""), 10)
            ) {
          ...
          if (
            parseInt(horario.replace(":", ""), 10) -
              parseInt(previosEnd.replace(":", ""), 10) >
            parseInt(selectedServicesTime.replace(":", ""), 10)
          ) {
            while (
              parseInt(availableTime.replace(":", ""), 10) <
              parseInt(horario.substr(0, 5).replace(":", ""), 10)
            ) {
              availableCards.push({
                enable: true,
        ...
        if (isTheLastAttendance(index)) {
          availableTime = SumHour(previosEnd, selectedServiceMinutes);
          if (
            LAST_HOUR - parseInt(previosEnd.replace(":", ""), 10) >=
            parseInt(selectedServicesTime.replace(":", ""), 10)
          ) {
  ...
  const getSelectedServicesTime = useCallback(() => {
    ...
    setSelectedServicesTime(
      `${hours.toString()}:${
        minutes > 10 ? minutes.toString() : "0".concat(minutes.toString())
      }`
    );
  }, [services]);
        ...
        {showModal && (
          <Portal>
            ...
            >
              ...
              <ModalSubtitle>
                {i18n.t("subtitle.youWishToCreateTheAttendance")}
              </ModalSubtitle>
              <ButtonContainer>
                <ButtonContent>
                  <Button
